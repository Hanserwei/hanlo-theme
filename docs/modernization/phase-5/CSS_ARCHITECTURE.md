...
| `base` | 元素基础规则与最后的 foundation 兼容模块 | 不接收新的页面或组件规则 |
...
| 关于页 | `pages/about-*.css` |
| Shiki、版权、资料卡 | `components/*.css` 条件入口 |
| 阅读模式 | `overrides/read-mode.css` 条件入口 |
...
1. 源文件位于 `src/css/components`、`src/css/pages` 或 `src/css/overrides`；
2. 在 `css-entries.json` 精确登记一次；
3. 全文件只包含一个与目录一致的 `@layer components/pages/overrides { ... }`；
4. 在对应 `src/*.html` 或片段中用 `theme.spec.version` 引用；
5. 为条件逻辑或路由补浏览器断言；
6. 不再从 `index.css` 导入同一文件。
...
3. 新模块直接进入目标 Cascade Layer；修改 foundation 时先补真实页面视觉覆盖。
4. 用 `--hanlo-*` 替换硬编码语义值，再用 `hanlo-*` 状态类降低选择器耦合。
5. 仅在静态、局部规则有明确收益时改用 `hl:` utility。
6. 运行 `pnpm check:css`、四象限视觉快照和真实 Halo 页面矩阵。

阶段 6 起，PostCSS AST 额外要求生效 `!important` 为 0，并对 ID selector、4+ combinator selector、
单 selector 最大 ID/组合器数量执行 `css-quality-budget.json` 只减不增预算。
