...
现有 Thymeleaf 片段、首屏同步主题引导和模板级 PJAX 初始化不能由 Vite HTML 解析器无差异处理。
阶段 1 在 `vite.config.ts` 中保留兼容桥，让 Halo 插件建立构建环境后逐字节复制
`src/**/*.html`。这是有意的模板保真边界：

- 保证首轮工程化不改变页面行为和视觉。
- 阶段 3 已迁移第一方业务脚本，模板级 PJAX 初始化和第三方依赖仍按现有方式运行。
- 移除兼容桥需要单独迁移模板构建语法，并重新验证 Thymeleaf 输出、稳定入口文件名和完整页面矩阵；
  它不是阶段 1–3 的未完成门禁。
