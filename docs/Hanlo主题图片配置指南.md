...
本文用于记录 Hanlo Theme 1.0.1 中「基础」「顶部」「侧栏」「页脚」四个设置组涉及的图片入口、推荐尺寸、内容建议、处理方式和上线检查流程，方便后续建站直接复用。
...
> 本文尺寸是根据 Hanlo Theme 1.0.1 的 Setting Schema、模板 CSS 和本次实际渲染结果整理的。主题升级后应重新检查 `settings.yaml` 和对应模板。
...
Hanlo Theme 1.0.1 的「基础」组没有专用 `$formkit: attachment` 图片字段，主要包含：
...
1. 宠物挂件：换成透明背景吉祥物。
2. 个人卡片贴纸：换成透明小图标。
3. 页脚中心 Logo：换成正式透明 Logo。
4. 上下班和框架徽标：换成真正的状态徽标或 SVG。
5. 音乐卡片：换成真实歌单或动态音乐卡片。
