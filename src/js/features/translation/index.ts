...
// Character table retained to preserve the theme's simplified/traditional conversion behavior.
  ...
  return {
    ...
    create: ({ config, resources }) => ({
      mount() {
        ...
        const snackbarData = config.Snackbar;
        ...
        // 此處可以更改為你想要顯示的文字
        const rightMenuMsgToTraditionalChinese =
          '<i class="haofont hao-icon-fanti" style="font-size: 19px;"></i><span>轉為繁體</span>';
        // 同上，但兩處均不建議更改
        const rightMenuMsgToSimplifiedChinese =
          '<i class="haofont hao-icon-jianti" style="font-size: 19px;"></i><span>转为简体</span>';

        let currentEncoding = defaultEncoding;
        const targetEncodingCookie = "translate-chn-cht";
        const storedEncoding = storage.get<number>(targetEncodingCookie);
        let targetEncoding =
          storedEncoding === 1 || storedEncoding === 2 ? storedEncoding : defaultEncoding;
        let translateButtonObject: HTMLElement | null;
        let translateRightMenuButtonObject: HTMLElement | null;
        const isSnackbar = config.Snackbar !== undefined;

        function setLang() {
          document.documentElement.lang = targetEncoding === 1 ? "zh-TW" : "zh-CN";
        }

        function translateText(txt: string): string {
        ...
        function translateBody(node: Node = document.body): void {
          if (node === translateButtonObject || node === translateRightMenuButtonObject) return;
          if (node instanceof Text) {
            node.data = translateText(node.data);
            return;
          }
          if (node instanceof HTMLElement) {
            if (node.matches("script, style, br, hr")) return;
            if (node.title) node.title = translateText(node.title);
            if (node instanceof HTMLImageElement && node.alt) node.alt = translateText(node.alt);
            if (
          ...
          node.childNodes.forEach(translateBody);
        }
        function translatePage() {
          if (targetEncoding === 1) {
            currentEncoding = 1;
            targetEncoding = 2;
            if (translateButtonObject) translateButtonObject.innerHTML = msgToTraditionalChinese;
            ...
            currentEncoding = 2;
            targetEncoding = 1;
            if (translateButtonObject) translateButtonObject.innerHTML = msgToSimplifiedChinese;
            if (translateRightMenuButtonObject)
          ...
          storage.set(targetEncodingCookie, targetEncoding, 2);
          setLang();
          translateBody();
        }
        function JTPYStr(): string {
        ...
        function simplified(cc: string): string {
          let str = "";
          const ss = JTPYStr();
          const tt = FTPYStr();
          for (let i = 0; i < cc.length; i++) {
        ...
        function translateInitialization(): void {
          translateButtonObject = document.getElementById("translateLink");
          const translateRightMenuButtonElement = document.getElementById("menu-translate");
          translateRightMenuButtonObject = translateRightMenuButtonElement;
          if (translateButtonObject || translateRightMenuButtonObject) {
            if (currentEncoding !== targetEncoding) {
              ...
              if (translateRightMenuButtonElement) {
                translateRightMenuButtonElement.innerHTML =
                  targetEncoding === 1
                    ? rightMenuMsgToSimplifiedChinese
                    : rightMenuMsgToTraditionalChinese;
              }
              setLang();
              resources.timeout(translateBody, translateDelay);
            }
            if (translateButtonObject)
              resources.listen(translateButtonObject, "click", translatePage);
            if (translateRightMenuButtonElement)
              resources.listen(translateRightMenuButtonElement, "click", translatePage);
          }
        }
